...
<html lang="en">
  ...
  const loadChapterContent = async (chId: string) => {
      setEditingChapterId(chId); // Open Editor immediately
  ...
  return (
    <div className="pb-20 bg-slate-50 min-h-screen">
      ...
      {activeTab === 'DASHBOARD' && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 mb-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  ...
                  <DashboardCard icon={FileJson} label="Rich Notes (HTML)" onClick={() => setActiveTab('CONTENT_NOTES')} color="teal" />
      ...
      {['CONTENT_PDF', 'CONTENT_VIDEO', 'CONTENT_MCQ', 'CONTENT_TEST', 'CONTENT_NOTES'].includes(activeTab) && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 animate-in slide-in-from-right">
              <div className="flex items-center gap-4 mb-6 border-b pb-4">
                  <button onClick={() => setActiveTab('DASHBOARD')} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200"><ArrowLeft size={20} /></button>
                  <h3 className="text-xl font-black text-slate-800">
                      {activeTab === 'CONTENT_PDF' ? 'PDF Study Material' : activeTab === 'CONTENT_VIDEO' ? 'Video Lectures' : activeTab === 'CONTENT_MCQ' ? 'Practice MCQs' : activeTab === 'CONTENT_NOTES' ? 'Rich Text Notes (HTML)' : 'Weekly Tests - Multi-Subject'}
                  </h3>
              </div>
...
Capital of India?       Mumbai  Delhi   Kolkata Chennai 2       Delhi is the capital`}
              ...
              {/* LIST VIEW (for PDF/VIDEO/MCQ/NOTES) */}
                      ...
                      {selChapters.map((ch) => (
                          <div key={ch.id} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                              ...
                              <button onClick={() => loadChapterContent(ch.id)} className="px-4 py-2 bg-blue-100 text-blue-700 font-bold rounded-lg text-xs hover:bg-blue-200">
                                  {activeTab === 'CONTENT_PDF' ? 'Edit Links' : activeTab === 'CONTENT_NOTES' ? 'Edit Notes' : 'Manage Questions'}
                              </button>
                          </div>
                      ))}
                  </div>
                      ...
                      {activeTab === 'CONTENT_VIDEO' && (
                          <div className="space-y-6 bg-gradient-to-br from-rose-50 to-pink-50 p-6 rounded-xl border border-rose-200">
                              <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <h4 className="font-bold text-rose-900">Video Playlist Manager</h4>
                              </div>
                              ...
                              <div className="bg-white p-4 rounded-xl border border-rose-100 shadow-sm mb-4">
                                  <div className="grid grid-cols-1 gap-3">
                                      <input 
                                          type="text" 
                                          id="newVideoTitle"
                                          className="w-full p-3 border border-rose-200 rounded-xl text-sm" 
                                      ...
                                      <button 
                                          onClick={() => {
                                              ...
                                              if (titleInput.value && urlInput.value) {
                                                  setVideoPlaylist([...videoPlaylist, { title: titleInput.value, url: urlInput.value }]);
                                                  titleInput.value = '';
                                                  urlInput.value = '';
                                              }
                                          }}
                                          className="bg-rose-600 text-white font-bold py-2 rounded-lg hover:bg-rose-700 text-sm"
                                      >
                                          + Add to Playlist
                                      </button>
                                  </div>
                              ...
                              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                                  {videoPlaylist.length === 0 && <p className="text-sm text-slate-400 text-center py-4">No videos added yet.</p>}
                                  {videoPlaylist.map((vid, i) => (
                                      <div key={i} className="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200">
                                          <div>
                                              ...
                                              <p className="text-[10px] text-slate-500 truncate max-w-[200px]">{vid.url}</p>
                                          </div>
                                          <button 
                                              onClick={() => setVideoPlaylist(videoPlaylist.filter((_, idx) => idx !== i))}
                              ...
                              {/* Legacy Support (Hidden but preserved in state logic if needed, but UI replaced) */}
                              
                              <button onClick={saveChapterContent} className="w-full bg-rose-600 text-white font-bold py-3 rounded-xl shadow hover:bg-rose-700 transition">💾 Save Playlist</button>
                          </div>
                      )}

                      {/* RICH TEXT NOTES EDITOR */}
                      {activeTab === 'CONTENT_NOTES' && (
                          <div className="space-y-6">
                              <div className="bg-teal-50 p-4 rounded-xl border border-teal-200 text-xs text-teal-800 mb-4">
                                  <strong>Tip:</strong> You can paste formatted text (colored, bold, tables) directly from Word or Google Docs here. Or write raw HTML.
                              </div>
                              
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Free Notes (HTML)</label>
                                  <div className="border rounded-xl overflow-hidden bg-white">
                                      {/* Simple Toolbar */}
                                      <div className="bg-slate-100 p-2 border-b flex gap-2">
                                          <button className="p-1 hover:bg-white rounded font-bold w-8" onClick={() => document.execCommand('bold')} title="Bold">B</button>
                                          <button className="p-1 hover:bg-white rounded italic w-8 font-serif" onClick={() => document.execCommand('italic')} title="Italic">I</button>
                                          <button className="p-1 hover:bg-white rounded underline w-8" onClick={() => document.execCommand('underline')} title="Underline">U</button>
                                          <div className="w-px bg-slate-300 mx-1"></div>
                                          <button className="p-1 hover:bg-white rounded text-blue-600 font-bold w-8" onClick={() => document.execCommand('foreColor', false, '#2563eb')} title="Blue Text">A</button>
                                          <button className="p-1 hover:bg-white rounded text-red-600 font-bold w-8" onClick={() => document.execCommand('foreColor', false, '#dc2626')} title="Red Text">A</button>
                                          <button className="p-1 hover:bg-white rounded text-green-600 font-bold w-8" onClick={() => document.execCommand('foreColor', false, '#16a34a')} title="Green Text">A</button>
                                          <button className="p-1 hover:bg-white rounded font-mono w-8 text-xs" onClick={() => document.execCommand('formatBlock', false, 'h3')} title="Heading">H3</button>
                                          <button className="p-1 hover:bg-white rounded font-mono w-8 text-xs" onClick={() => document.execCommand('insertUnorderedList')} title="List">•</button>
                                      </div>
                                      <SimpleRichTextEditor
                                          className="p-4 min-h-[150px] outline-none text-sm prose prose-sm max-w-none"
                                          value={editConfig.freeNotesHtml || ''}
                                          onChange={html => setEditConfig({...editConfig, freeNotesHtml: html})}
                                      />
                                  </div>
                                  <textarea 
                                      className="w-full mt-2 p-2 text-[10px] font-mono border rounded bg-slate-50 text-slate-400 h-20" 
                                      placeholder="Raw HTML Source (for advanced editing)"
                                      value={editConfig.freeNotesHtml || ''}
                                      onChange={e => setEditConfig({...editConfig, freeNotesHtml: e.target.value})}
                                  />
                              </div>

                              <div className="border-t pt-4">
                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Premium Notes (HTML)</label>
                                  <div className="border rounded-xl overflow-hidden bg-white border-purple-200">
                                      <div className="bg-purple-50 p-2 border-b border-purple-200 flex gap-2">
                                          <span className="text-xs font-bold text-purple-700 uppercase self-center mr-2">Premium Content</span>
                                          <button className="p-1 hover:bg-white rounded font-bold w-8" onClick={() => document.execCommand('bold')}>B</button>
                                          <button className="p-1 hover:bg-white rounded italic w-8 font-serif" onClick={() => document.execCommand('italic')}>I</button>
                                          <button className="p-1 hover:bg-white rounded text-blue-600 font-bold w-8" onClick={() => document.execCommand('foreColor', false, '#2563eb')}>A</button>
                                          <button className="p-1 hover:bg-white rounded text-red-600 font-bold w-8" onClick={() => document.execCommand('foreColor', false, '#dc2626')}>A</button>
                                      </div>
                                      <SimpleRichTextEditor
                                          className="p-4 min-h-[150px] outline-none text-sm prose prose-sm max-w-none"
                                          value={editConfig.premiumNotesHtml || ''}
                                          onChange={html => setEditConfig({...editConfig, premiumNotesHtml: html})}
                                      />
                                  </div>
                                   <textarea 
                                      className="w-full mt-2 p-2 text-[10px] font-mono border rounded bg-slate-50 text-slate-400 h-20" 
                                      ...
                                      value={editConfig.premiumNotesHtml || ''}
                                      onChange={e => setEditConfig({...editConfig, premiumNotesHtml: e.target.value})}
                                  />
                              </div>

                              <button onClick={saveChapterContent} className="w-full bg-teal-600 text-white font-bold py-3 rounded-xl shadow hover:bg-teal-700">Save HTML Notes</button>
                          </div>
                      )}
