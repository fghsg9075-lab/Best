    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white w-full max-w-sm rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden relative">
            ...
            <div className="p-6">
                ...
                {/* OPTION 3: RICH HTML NOTES */}
                <div className="grid grid-cols-2 gap-4 mb-4">
                    {/* OPTION 1: FREE HTML */}
                    <button 
                        onClick={() => onSelect('NOTES_HTML_FREE')}
                        className="p-4 rounded-2xl border-2 border-teal-100 bg-teal-50/50 hover:bg-teal-100 hover:border-teal-300 flex flex-col items-center text-center gap-3 transition-all"
                    >
                        <div className="bg-teal-600 text-white p-3 rounded-xl shadow-lg shadow-teal-200">
                             <FileJson size={24} />
                        </div>
                        <div>
                            <div className="font-bold text-slate-800 text-sm">Rich Notes</div>
                            <div className="text-[10px] text-slate-500">Read in App</div>
                        </div>
                        <div className="text-[9px] font-bold bg-white px-2 py-1 rounded border border-teal-200 text-teal-700">
                            FREE
                        </div>
                    </button>

                    {/* OPTION 2: PREMIUM HTML */}
                    <button 
                        onClick={() => hasCredits(5) && onSelect('NOTES_HTML_PREMIUM')}
                        className={`p-4 rounded-2xl border-2 flex flex-col items-center text-center gap-3 transition-all ${
                            hasCredits(5) 
                            ? 'border-pink-100 bg-pink-50/50 hover:bg-pink-100 hover:border-pink-300' 
                            : 'border-slate-100 bg-slate-50 opacity-70'
                        }`}
                    >
                        <div className="bg-pink-600 text-white p-3 rounded-xl shadow-lg shadow-pink-200">
                             <Star size={24} />
                        </div>
                        <div>
                            <div className="font-bold text-slate-800 text-sm">Pro Notes</div>
                            <div className="text-[10px] text-slate-500">Detailed HTML</div>
                        </div>
                        <div className="text-[9px] font-bold bg-white px-2 py-1 rounded border border-pink-200 text-pink-700">
                            {isAdmin ? 'FREE' : '5 CR'}
                        </div>
                    </button>
                </div>
                ...
                {/* VIDEO OPTION (NEW) */}
                <button 
                    onClick={() => hasCredits(5) && onSelect('VIDEO_LECTURE')}
                ...
                >
                    <div className="flex items-center gap-3">
                        <div className="bg-red-600 text-white p-3 rounded-xl shadow-lg shadow-red-200">
                            <BookOpen size={24} className="hidden" /> {/* Fallback icon if needed */}
